...
# Streamlit Application Specification: LIME for Explaining Stock Predictions

## 1. Application Overview

The **LIME for Explaining Stock Predictions** application is designed for CFA Charterholders and Investment Professionals to understand and trust black-box quantitative stock scoring models. It addresses the critical need for explainability in investment recommendations, enabling users to perform due diligence and communicate rationales to investment committees.

The application guides the user through a structured workflow:

1.  **Introduction & Setup**: Introduce the problem of black-box models, set up a simulated stock scoring environment, and train an XGBoost regressor as the proprietary model.
2.  **LIME Local Explanations**: Select a specific stock (e.g., a top-performing one) and generate a LIME explanation, translating technical feature contributions into a clear, PM-friendly investment rationale.
3.  **LIME Stability Analysis**: Assess the reliability of LIME explanations by running them multiple times for the same stock, visualizing feature weight distributions and quantifying stability using the Coefficient of Variation.
4.  **SHAP vs. LIME Comparison**: Compare LIME explanations with SHAP explanations for the same stock, highlighting areas of agreement or disagreement and quantifying rank correlation.
5.  **XAI Method Selection Guide**: Provide a strategic framework (decision matrix and heuristics) for choosing between LIME and SHAP based on model type, explanation needs, and regulatory context.

This workflow simulates a real-world scenario where a Portfolio Manager utilizes XAI techniques to gain confidence in model-driven investment recommendations and fulfill regulatory and ethical obligations for due diligence.
...
from scipy.stats import spearmanr # Needed for re-calculating Spearman if comparison_df is used for plotting rank correlation directly.
...
from source import (
    ...
    test_lime_stability,
    compare_shap_lime,
    # The 'language_map' variable is not explicitly returned by a function,
    # but is used within 'generate_investment_rationale'. If it were needed
    # for display purposes, it would need to be passed or accessed.
    # For now, assume generate_investment_rationale handles it internally.
)
```

### `st.session_state` Design

The application uses `st.session_state` to maintain persistence across user interactions and page changes.
...
if 'initialized' not in st.session_state:
    ...
    st.session_state.selected_stock_idx = None # Will be set to top_stock_idx_in_test after setup
    st.session_state.stock_indices_options = [] # List of available stock indices for selection

    # Dictionaries to cache explanation results for different stocks
    st.session_state.lime_explanation_data = {} # Key: stock_idx, Value: (explanation_obj, contrib_df, prediction)
    st.session_state.rationale_data = {}        # Key: stock_idx, Value: summary_str
    st.session_state.stability_data = {}         # Key: stock_idx, Value: (stab_df, plot_df_filtered)
    st.session_state.comparison_data = {}        # Key: stock_idx, Value: (shap_contrib_df, lime_contrib_df, rank_corr_coeff, comparison_df)

    st.session_state.n_stability_runs = 10 # Default number of runs for stability analysis
...
*   **`initialized`**: Set to `True` after `setup_stock_scoring_environment` is successfully executed.
*   **Model and Data (`model`, `X_train`, etc.)**: Updated once during the "Introduction & Setup" page after calling `setup_stock_scoring_environment`. Read by all subsequent pages requiring the model or data.
*   **`lime_explainer`**: Initialized during the "Introduction & Setup" page. Read by pages needing LIME explanations.
*   **`top_stock_idx_in_test`, `bottom_stock_idx_in_test`**: Initialized during setup. Used as default options for `selected_stock_idx`.
*   **`selected_stock_idx`**: Updated by user selection via `st.selectbox`. Its value determines which stock's data is retrieved and processed for explanations, stability, or comparison.
*   **Cached Data (`lime_explanation_data`, `rationale_data`, etc.)**:
    *   **Update**: When a user selects a `selected_stock_idx` and triggers an explanation, stability analysis, or comparison, the results are stored in the respective `st.session_state` dictionary using `selected_stock_idx` as the key.
    *   **Read**: Before performing a computation for a `selected_stock_idx`, the app checks if the result already exists in the cache. If it does, the cached result is used to avoid re-running expensive computations. This simulates a persistent workflow where explanations are remembered.
*   **`n_stability_runs`**: Updated by a `st.number_input` widget on the "LIME Stability Analysis" page.

### Application Structure and Flow (Page-by-Page)

The application uses a sidebar selectbox for navigation, simulating a multi-page experience.
...
if not st.session_state.initialized:
    ...
    if st.button("Setup Environment and Train Model"):
        with st.spinner("Setting up environment and training model..."):
            ...
            st.session_state.factors_df = factors_df_full # Store the full DF if needed, or just X_test
            ...
            st.session_state.selected_stock_idx = top_stock_idx_in_test # Default to top stock
            st.session_state.stock_indices_options = sorted(list(X_test.index.values)) # Use indices from X_test directly
            st.session_state.initialized = True
        st.success("Environment setup complete! Model trained and explainer initialized.")
        st.experimental_rerun() # Rerun to update UI after initialization
...
**UI Interactions & Function Calls:**

*   **Button**: `st.button("Setup Environment and Train Model")`
*   **Function Call**: `setup_stock_scoring_environment()` is called.
*   **State Updates**: `st.session_state.model`, `st.session_state.X_train`, `st.session_state.y_train`, `st.session_state.X_test`, `st.session_state.y_test`, `st.session_state.feature_cols`, `st.session_state.factors_df`, `st.session_state.lime_explainer`, `st.session_state.top_stock_idx_in_test`, `st.session_state.bottom_stock_idx_in_test`, `st.session_state.selected_stock_idx`, `st.session_state.stock_indices_options`, `st.session_state.initialized` are updated.

---

#### Page 2: LIME Local Explanations

**Pre-requisite Check:** Ensure `st.session_state.initialized` is `True`.
...
if st.session_state.initialized:
    ...
    st.session_state.selected_stock_idx = st.selectbox(
        ...
        index=st.session_state.stock_indices_options.index(st.session_state.selected_stock_idx)
    )
    ...
    # Generate and display investment rationale (V2)
...
*   `explain_stock_lime(st.session_state.model, st.session_state.X_test, st.session_state.selected_stock_idx, st.session_state.lime_explainer, st.session_state.feature_cols, stock_name=...)`
*   `generate_investment_rationale(explanation_obj.as_list(), prediction, stock_name=...)`

**State Updates:**

*   `st.session_state.selected_stock_idx` is updated by the `st.selectbox`.
*   `st.session_state.lime_explanation_data[stock_key]` is updated with the LIME explanation, contributions, and prediction.
*   `st.session_state.rationale_data[stock_key]` is updated with the generated rationale summary.
...
if st.session_state.initialized:
    ...
    if stock_key in st.session_state.stability_data:
        ...
        # Plot stability box plots (V3 part 2)
...
*   `test_lime_stability(st.session_state.model, st.session_state.X_test, st.session_state.selected_stock_idx, st.session_state.lime_explainer, st.session_state.feature_cols, n_runs=st.session_state.n_stability_runs)`

**State Updates:**

*   `st.session_state.selected_stock_idx` is updated.
*   `st.session_state.n_stability_runs` is updated.
*   `st.session_state.stability_data[stock_key]` is updated with the stability DataFrame and plot data.
...
if st.session_state.initialized:
    ...
    if stock_key in st.session_state.comparison_data:
        ...
        st.markdown(f"**Feature Contribution & Rank Comparison (V4 - Table):**")
        ...
        # Plot SHAP vs. LIME Rank Comparison (V6)
...
*   `compare_shap_lime(st.session_state.model, st.session_state.X_test, st.session_state.selected_stock_idx, st.session_state.lime_explainer, st.session_state.feature_cols, stock_name=...)`

**State Updates:**

*   `st.session_state.selected_stock_idx` is updated.
*   `st.session_state.comparison_data[stock_key]` is updated with the SHAP/LIME comparison data and rank correlation.

---

#### Page 5: XAI Method Selection Guide

**Pre-requisite Check:** None. This page is informational.
...
st.subheader("SHAP vs. LIME Decision Matrix (V5)")
st.markdown(
    """
| Criterion               | SHAP                                                                   | LIME                                                                         |
| :---------------------- | :--------------------------------------------------------------------- | :--------------------------------------------------------------------------- |
...
**Function Calls:** None. This page is static informational content.
